...
class LogInView(GuestOnlyView, FormView):
    ...
    def get_form_class(
        **kwargs: Any,
    ) -> type[SignInViaEmailForm] | type[SignInViaEmailOrUsernameForm] | type[SignInViaUsernameForm]:
    ...
    def form_valid(self, form: SignIn) -> HttpResponse:
...
class ActivateView(View):
    @staticmethod
    def get(request: HttpRequest, code: str) -> HttpResponse:
...
class ResendActivationCodeView(GuestOnlyView, FormView):
    ...
    @staticmethod
    def get_form_class(**kwargs: Any) -> type[ResendActivationCodeViaEmailForm] | type[ResendActivationCodeForm]:
        if settings.DISABLE_USERNAME:
            return ResendActivationCodeViaEmailForm

        return ResendActivationCodeForm

    def form_valid(self, form: ResendActivationCodeForm) -> HttpResponse:
...
class RestorePasswordView(GuestOnlyView, FormView):
    ...
    @staticmethod
    def get_form_class(**kwargs: Any) -> type[RestorePasswordViaEmailOrUsernameForm] | type[RestorePasswordForm]:
        if settings.RESTORE_PASSWORD_VIA_EMAIL_OR_USERNAME:
            return RestorePasswordViaEmailOrUsernameForm

        return RestorePasswordForm

    def form_valid(self, form: RestorePasswordForm) -> HttpResponse:
        user: User = form.user_cache
...
class ChangeProfileView(FormView, LoginRequiredMixin):
    ...
    def form_valid(self, form: ChangeProfileForm) -> HttpResponse:
        initial = super().get_initial()
        user = self.request.user
        if isinstance(user, AnonymousUser):
            return initial
...
class ChangeEmailView(FormView, LoginRequiredMixin):
    ...
    def form_valid(self, form: ChangeEmailForm) -> HttpResponse:
        user = self.request.user
        if isinstance(user, AnonymousUser):
            return redirect("index")
...
class RemindUsernameView(FormView, GuestOnlyView):
    ...
    def form_valid(self, form: RemindUsernameForm) -> HttpResponse:
        user: User = form.user_cache
